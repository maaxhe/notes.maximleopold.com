...
const CONFIG = {
  ...
  VAULT_PATH: process.env.VAULT_PATH || '/Users/maxmacbookpro/Library/Mobile Documents/iCloud~md~obsidian/Documents/Brain online',
...
function collectAssets(): Map<string, string> {
  ...
  for (const assetsDir of CONFIG.ASSETS_DIRS) {
    ...
    walkDirectory(fullPath, (filePath) => {
      ...
      const isAsset = ['.png', '.jpg', '.jpeg', '.gif', '.svg', '.pdf', '.mp4', '.webm'].includes(ext);
...
function main(): void {
  ...
  if (!fs.existsSync(CONFIG.VAULT_PATH)) {
    ...
    console.error('Please update VAULT_PATH in the script or set the VAULT_PATH environment variable.');
    process.exit(1);
  }
