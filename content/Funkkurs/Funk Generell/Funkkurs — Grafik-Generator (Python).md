...
So benutzt du es: Skript **lokal** mit `python3 funkkurs_figs.py` ausführen (braucht `matplotlib` + `numpy`). Die PNGs landen direkt im Vault-Root und werden in den Modulen per `![[…]]` eingebettet. Werte im Skript anpassen, neu ausführen, und die Grafiken aktualisieren sich.

Nicht im Browser-Python (Pyodide) ausführen!
...
Erweitern lässt sich das leicht: Weitere Ideen sind eine Kanalbelegungs-Übersicht als Grafik, ein Tortendiagramm „Sende- vs. Empfangszeit" oder eine Karte der Küstenfunkstellen. Einfach neue `fig`-Blöcke nach dem Muster ergänzen.
