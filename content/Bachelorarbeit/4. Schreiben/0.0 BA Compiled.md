...
We utilized the pre-computed HCP Parcellated Time Series (HCP1200 Release). In this dataset the preprocessed BOLD fMRI signal has already been spatially averaged across all vertices within each distinct parcel of the HCP MMP1 atlas ([[Glasser et al. (2016) - Nature]]). For each subject, the mean BOLD time series were extracted for all seed regions (FEF, IFJa, IFJp, 55b, 44, 45) as well as all auditory target regions defined in section 3.2.
## 3.3.3 Connectivity Matrix Construction
Pairwise FC values were computed as Pearson correlation coefficients between the extracted BOLD time series of each seed region and all 360 parcels of the HCP MMP1 atlas. This led to a connectivity vector representing 
