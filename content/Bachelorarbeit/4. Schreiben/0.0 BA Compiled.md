...
[Drüber schauen!!!]
Brain-behavior relationships were assessed using a cross-validated linear correlation framework. For each behavioral measure, the RSFC profile across the predefined ROI network served as the multivariate predictor. The connectivity values were derived from partial correlation matrices computed across the N = 371 subsample, reflecting direct functional coupling between ROIs with shared variance removed. The prediction model itself was fitted using linear correlation between the connectivity predictor matrix and the behavioural outcome variable.

Separate models were computed for the full network (all ROIs), the ventral target subset, and the dorsal target subset, enabling stream-specific predictions to be tested independently. For the Language Task Median RT, an additional analysis was run on the full correlation matrix of the N = 812 resting-state sample with K = 10 cross-validation to test whether the effect holds beyond the matched behavioral subsample.
