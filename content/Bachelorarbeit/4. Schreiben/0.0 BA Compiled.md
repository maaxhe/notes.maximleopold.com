...
**Areas A4 and A5**. Both showed robust coupling with IFJa; only A4 showed slight connectivity with FEF. Glasser's argument where A4 and A5 are both activated in LANGUAGE STORY contrast - which is connected to IFG (44, 45, 47l) - supports the membership to the 'what'-stream. Since A5 also is inferior to A5, this leads A5 more to the ventral pathway which reflect the connectivity pattern. Whereas A4 might rather functional as a router for information flow to higher level areas in mainly the 'what'-stream. Rolls' argument of A4/A5 demonstrating EC with MT and MST does not conflict with our results, since this reflects an indirect route to FEF. This might reflect a divergence hub, distributing information to both 'what' and 'where'-pathways. 

**PSL**. The PSL shows an interesting pattern, where it couples with IFJa left-hemispherically and with FEF right-hemispherically. Additionally, Dureux argues that PSL is not respoinsive to auditory stimuli at all, which might replect PSL as an abstract, modality-independent level, not on acoustic features. On the other hand, Rolls 2023 shows directional EC from STS regions into PSL. PSL might serve as a spuramodal connector or a high-levek convergence hub, left side to semantic identitiy via IFJa, while right side links to spatial attention via FEF. It is interesting that a region which does not respond to any acoustic stimuli couples strongly with both prefrontal attention hubs, which could imply that top-down connections do not require bottom-up auditory drive at the target (what target do you mean here?)

**STV**. Comparing to FEF, IFJa shows stringer coupling to STV than FEF
