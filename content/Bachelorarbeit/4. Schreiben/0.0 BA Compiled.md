...
The prefrontal cortex not only is the most important brain region for decision making, m but also serves as the main hub for top-down attention. When we decide to look at the squirrel sitting on the table, it is our prefrontal cortex that first decides and then initiates the directing of the yes and also the visual system to attend to the squirrel. While focusing on the squirrel, we hear the squirrel making noises - we then attend to the auditory stimuli as well. The question still remains which parts of the prefrontal cortex direct attention to the auditory stream. My thesis will examine the scope in which the prefrontal cortex directs attention top-down in different modalities. But first, we need to dive into the existing literature about the visual stream hypothesis. 
...
We hypothesize that the resting-state functional connectivity of the auditory 'what' and 'where'-streams shows a clear dissociation: the auditory 'where'-stream preferentially connects to the Frontal Eye Field (FEF), and the auditory 'what'-stream to the anterior Inferior Frontal Junction (IFJa).
