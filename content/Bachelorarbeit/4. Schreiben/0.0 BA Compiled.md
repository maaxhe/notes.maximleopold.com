...
The prefrontal cortex serves as a primary hub for top-down attentional control, directing perceptual processing toward behaviorally relevant stimuli while suppressing competing input across sensory modalities. While the mechanisms by which prefrontal regions coordinate visual attention have been extensively characterized, the question of which prefrontal areas perform the equivalent function for auditory processing — and whether they are organized according to the same principles — remains largely unresolved. The present thesis addresses this gap by examining the resting-state functional connectivity of established visual prefrontal control hubs within the auditory domain. The following sections trace the conceptual path from the visual two-stream hypothesis to its auditory analogue, establishing the theoretical foundation for the analyses in Chapters 3 and 4. 
...
This task was chosen as the closest available test of spatial processing capacity, linked to the FEF's dorsal stream. Because the HCP dataset does not include an auditory spatial task, this visual task tests the supramodal hypothesis. Participants might solve these tasks via object recognition rather than spatial navigation, which might reduce observed effects in the dorsal network. 
### Words-in-Noise (NIH Toolbox Noise Comparison)
The NIH Toolbox Words-in-Noise test presents spoken English words masked by continuous speech-like background noise at an adaptive signal-to-noise ratio ([[Barch et al. (2013)]]). Responses are given by selecting one of four pictures. This test was included as a control paradigm indexing low-level acoustic signal filtering rather than high-level semantic processing.
...
The story accuracy (Acc) model yielded a qualitatively different picture. The full network predicted story Acc robustly ($R = 0.38$, $p < 0.001$), and both the ventral ($R = 0.32$, $p < 0.001$) and dorsal ($R = 0.24$, $p < 0.001$) submodels reached significance. Crucially, area 45 appeared as a *negative* predictor of accuracy: stronger Broca's area coupling was associated with *lower* accuracy. Given the trivial vocabulary of the Aesop fable narratives, this pattern might point towards an over-engagement of deep semantic and syntactic machinery disrupting fluent story comprehension — an interpretation further supported by the ceiling effect observed in the larger full-sample analysis (full812, n.s.). An alternative interpretation cannot be excluded, however: given the near-ceiling accuracy, residual variance may reflect attentional lapses rather than a systematic processing cost, rendering the negative Area 45 contribution partially susceptible to noise-fitting. The dorsal model's contribution to accuracy prediction, driven by areas 43, 7PC, and PF, likely reflects global attentional demand rather than stream-specific semantic processing. Reaction time is therefore the more interpretable and stream-specific metric for the "what" stream hypothesis.
...
For Place RT, the dorsal model produced an artefactual result ($R = -1.00$), a consequence of matrix rank collapse under partial correlation with an insufficient number of ROIs — this result is excluded from interpretation (see section 5.7). The ventral model yielded a marginally significant prediction ($R = -0.11$, $p = 0.035$), with area STSda contributing positively and IFJa negatively. The negative contribution of IFJa — that is, stronger IFJa connectivity associated with faster Place RT — is consistent with the semantic hijacking interpretation advanced above. If participants resolve the Place task via object recognition rather than spatial navigation, then IFJa engagement reflects its established role as semantic controller ([[Soyuhos, O., & Baldauf, D. (2023)]], [[Bedini, M., & Baldauf, D. (2021)]]), not a domain-general executive response. This reading is reinforced by area 47l driving Place Accuracy in the same ventral model: both predictors point to a semantic processing strategy as the dominant mechanism. The apparent extension of IFJa into a visuo-spatial task therefore does not challenge the ventral stream specificity observed in Section 4.6.1, but instead provides convergent evidence that the task itself elicits ventral rather than dorsal processing. The full-network model ($R = -0.20$, $p < 0.001$) identified MT and TA2 as the strongest negative predictors of Place RT — that is, stronger connectivity to visual motion and auditory association cortex was associated with faster responses — while PSL emerged as the leading positive predictor, suggesting that stronger perisylvian language area coupling was associated with slower Place RT.
...
This double dissociation — ventral specificity for semantic RT, parieto-opercular dominance for acoustic filtering — suggests a hierarchical organization in which the "cocktail party" problem of physical signal separation is solved at a lower, hardware-level auditory processing stage before the IFJa-anchored prefrontal network engages in meaning extraction.
