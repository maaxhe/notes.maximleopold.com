...
Statistical significance of RSFC estimates was assessed using the paired Wilcoxon signed-rank test, an alternative to the Gaussian-based paired t-test. This test was chosen because RSFC values derived from Pearson correlations do not necessarily follow a Gaussian distribution across subjects ([[Soyuhos, O., & Baldauf, D. (2023)]]) For the single-seed analyses, a one-tailed test was applied, evaluating whether connectivity with the seed region exceeded the mean whole-brain connectivity. For contrast analyses, a two-tailed test was used to determine whether connectivity was significantly higher for one seed than for the other. 
...
**A4 and A5**. We placed areas A4 in the 'where'-stream and A5 in the 'what'-stream (Section 3.2.5), given the conflicting evidence in the literature ([[Rolls et al. (2023) - Cerebral Cortex]], [[Glasser et al. (2016) - Nature]]). Partial correlation analysis resolves this conflict empirically by showing robust and bilateral connectivity of both areas A4 and A5 with IFJa (A4: left $z = 8.25$, $\text{mean} = 0.01$; right $z = 8.76$, $\text{mean} = 0.01$; A5: left $z = 11.63$, $\text{mean} = 0.01$; right $z = 6.96$, $\text{mean} = 0.01$), while showing no meaningful coupling with FEF. The only exception is a weak, left-lateralized A4-FEF connection ($z = 4.06$, $\text{mean} = 0.01$) in the single seed analysis with FEF. This pattern positions both A4 and A5 as 'what'-stream gateways under potential IFJa top-down control. 

**PBelt**. PBelt was grouped alongside A4 as a potential dorsal gateway in the pre-analysis classification (Section 3.2.5). In the single-seed IFJa analysis, PBelt shows only weak coupling (right: $z = 4.79$, $\text{mean} = 0.01$). Partial correlation reduces this further to near-equivalent values for both prefrontal seeds (IFJa: right $z = 3.05$, $\text{mean} = 0.01$; FEF: left $z = 2.92$, $\text{mean} = 0.00$). The absence of a dominant prefrontal partner, combined with near-zero mean connectivity, indicates that PBelt's functional coupling is insufficient for reliable stream assignment.
...
A complete listing of all target regions of interest with their final stream assignments — including areas reclassified on the basis of partial correlation results (Sections 4.4.1–4.4.3) — is provided in Appendix Table A2.
...
**STV**. In the comparative partial correaltion analysis, IFJa shows stronger coupling to STV than FEF. Since STV couples with both FEF and IFJa almost equally in mean connectivity, STV places itself as a pretty important player in top-attention. This could imply a multimodality of STV connecting spatial inforamtion with semantic, also maybe right STV could therefore be a gateway for emotional voice content into the qhat-stream reflecting Frühholz take on these regions  ([[Frühholz (2015) - NeuroImage]])
