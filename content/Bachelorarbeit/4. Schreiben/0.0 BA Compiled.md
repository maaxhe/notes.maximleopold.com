...
The prefrontal cortex not only is the most important brain region for decision making, but also serves as the main hub for top-down attention in a multi-modal framework. For vision humans can pay attention only to the most important parts and blend out irrelevant visual features in the background (trees, parking cars). For the auditory domain, humans perform similar mechanisms. We have a profound ability in distinguishing spectral features in a rich auditory environment. In a train station, we can differentiate between an announcement and blend out the heavy rolling of suitcases on the tiles of the entrance hall. The question still remains which parts of the prefrontal cortex direct attention to the auditory stream. My thesis will examine the scope in which the prefrontal cortex directs attention top-down in different modalities. But first, we need to dive into the existing literature about the visual stream hypothesis. 
...
This sensorimotor dimension was elaborated by Hickok and Poeppel (2004, 2007), who proposed that the dorsal stream maps acoustic speech signals onto articulatory representations, projecting from the STG via the Sylvian parietal temporal area (Spt) to frontal regions including Broca's area (pars opercularis, Area 44, and pars triangularis, Area 45). Rather than contradicting the spatial account, this suggests that the dorsal pathway supports a broader **spatial-motor** function — localising sounds in space and coupling that information with motor action plans.
...
Spatial processing in the temporo-parietal cortex is predominantly right-lateralised, consistent with the well-documented asymmetry of spatial neglect in patients with right-hemispheric damage ([[Rauschecker & Scott (2009)]]). In contrast, Hickok & Poeppel's dorsal language model that links area Spt to Broca's area is strongly left-dominant ([[Hickok & Poeppel (2007)]]). This hemispheric dissociation suggests that the "where"-stream operates in parallel in both hemispheres with different functional emphases: right-lateralised spatial and motion processing, and left-lateralised sensorimotor integration. 
...
### a-priori Table 2: Where & How Stream (Dorsal)
...
### a-priori Table 3: What pathway (ventral)
...
The right hemisphere reveals an exclusive direct connection to the PSL ($z = 21.37$, $\overline{r} = 0.03$), which is entirely absent in the left hemisphere's partial profile, indicating a right-lateralised pathway for abstract spatial-auditory processing ([[Rolls (2022) - NeuroImage]]).
...
Partial correlation reveals bilateral coupling with auditory areas A4 (left $z = 8.25$; right $z = 8.76$) and A5 (left $z = 11.63$; right $z = 6.96$), along with right lateralised PBelt connections ($z = 4.79$). Additionally, robust multimodal convergence areas like STV (left $z = 18.90$; right $z = 18.55$) and TPOJ1 are deeply integrated into the IFJa network, functioning as a combination of auditory identity with other sensory modalities. Additionally, PSL couples bilaterally with IFJa in both hemispheres ($\overline{r} = 0.01$, right: $z=13.06$, left: 9.65)
...
To summarise the global connectivity findings, there is converging evidence that FEF and IFJa function as prefrontal attention hubs not only for vision, but also for audition. We argue for a stronger interpretation than solely a multimodal one: FEF and IFJa are supramodal. [[Spagna et al. (2015)]] proposed that executive control of attention operates supramodally, coordinating behaviour independent of modalities through shared neural mechanisms, instead of modality-specific processes as alerting and orienting. 
Our RSFC results are consistent with this view at the anatomical level: FEF and IFJa do not merely respond to both vision and audition, but implement the same organisational principle: spatial versus non-spatial top-down control, independent of the sensory modality ([[Bedini & Baldauf (2021)]]). This is consistent with the evolutionary argument that visuospatial attention systems are preserved in primates and later also recruited other sensory domains [[Bedini & Baldauf (2021)]]. Whether these RSFC coupling patterns reflect true genuine top-down control signals still needs to be explored with directed connectivity methods ([[De Vries & Baldauf (2021) - Journal of Neuroscience]]). The present findings represent a first step towards this hypothesis, further implications are discussed in Section 5.6.

---

# 5.2 Re-evaluating the auditory where stream
## 5.2.1 The Spatial Link ('Where'-Stream)
In the visual domain, FEF is strongly coupled with 7PC/7Am; in the auditory domain, however, FEF recruits inferior parietal regions (PF, PFop, PFcm) in the partial correlation, while still correlating with motion-sensitive area MST, possibly reflecting FEF's role in supramodal motion integration. This dissociation aligns with the literature and with known differences in auditory and visual spatial network topology. The direct coupling with multisensory convergence hub TPOJ1 further supports this argumentation. Together, these patterns suggest that FEF functions as a shared supramodal controller that recruits modality-appropriate parietal regions. [[Rauschecker & Scott (2009) - Nature Neuroscience]] show that the auditory dorsal stream is spatial, our analysis adds the prefrontal component to this stream. 
...
Comparing 55b to FEF, 55b shows strong coupling with both FEF and IFJa, as well as regions of the 'what'-stream. 55b maintains direct functional access to A4 and A5, whereas FEF does not, suggesting a functional division of labour: FEF encodes higher-order spatial information, while 55b might serve as its acoustic relay into the auditory hierarchy. 55b may receive spatial commands from FEF and translate them into the semantic-auditory system. This is further supported by 55b's stronger coupling with motor areas relative to FEF. Together, these findings suggest the dorsal stream is not monolithic but reflects a dissociation between FEF as a supramodal spatial-motor controller and 55b as an auditory-language integration hub.
Additionally, areas 44 and 45 show no coupling with FEF, suggesting that the dorsal language network is distinct from the spatial attention network directed by FEF. 
...
We used the existing fMRI in-scanner and out-of-scanner tasks to assess whether behavioural differences predict the connectivity within our set of ROIs in the auditory domain. The best approach for such a task would be to create an own in-scanner task to test our hypotheses. However, we chose only among the existing tasks from the HCP battery, due to feasibilty and also due to the large subject pool of 371 subjects generated in these HCP studies. The latter makes predicitve power a lot more accurate. 
For the testing whether IFJa coupling with the auditory what-pathway is predicted by behaviour, we chose the semantic "Langueg Task Story, median reaciton time (RT)" as a predictor. 
In general this behavioural tasks are for RSFC, the results remain to be cheked by better fitting MEG studies. 

**Auditory what-stream.** Reaction Time tested whether the ROIs predicited the behavioural tasks of story median RT well, but not for where-stream ROIs.

**Auditory where-stream.** [[Salmi (2009)]] shows that FEF is activated in auditory spatial attention tasks, therefore we should see a pattern in which FEF RSFC predicts spatial performance. Since spatial hearing tasks do not exist in the HCP battery, we solely relied on very broad features of the where-stream with acoustic signal filtering and visuo-spatial working memory tasks. 
Even though only broadly related we received results showing differences between where- and what-pathway. The what-pathway fails to be predicted by acoustic signal filtering while the where-stream shows predictors involved in spatial and motor tasks. For working memory we did not find significant results. This could be due to IFJa and other prefrontal areas in this study assigned to the auditory what-pathway also playing a role in working memory. This is a weak, but still a hint that a better-matched desinged study could be worth to perform to find out more about the auditory where-stream. 
Also, the visual Place task (Working Memory) does not recruit the auditory spatial network, but rather shows that the 'what'-streams predicts performance in Place RT task. As mentioned in 3.4.2.2, this could be dure to participants remembering the *kind* of object and rather, where the object used to be in space. This is supported by IFJa's role in working memory. Also, we tested for the auditory stream and here, there were no auditory stimuli present. 
Additionally, RSFC measures structural coupling tendencies and not functional activation during spatial attention
--→nochmal nachschauen, weil ich denke, dass es in-scanner ist richtig?
## Right Hemisphere Lateralisation
For the predictive modelling, we only find significant results within the right hemisphere; results from the left hemisphere are consistently non-significant across all stream models and tasks. This may reflect the strong language dominance of the left hemisphere: because both what- and where-stream ROIs maintain broad functional coupling with language regions there, the partial correlation cannot isolate stream-specific variance — both streams contribute similarly, leaving no differential signal to detect. In the right hemisphere, the functional division between spatial and semantic processing is more distinct, allowing the dissociation to emerge.

An additional explanation lies in the nature of the task itself. The HCP Language Story task (Aesop's Fables) operates at the level of narrative and discourse rather than phonology or syntax, and such discourse-level semantic integration is characteristically right-lateralised ([[Beeman & Chiarello (1998)]]; [[Jung-Beeman (2005)]]). Classical left lateralisation applies to word- and sentence-level processing; at the level of narrative inference and moral reasoning, the right hemisphere is the more relevant predictor. This is consistent with our finding that the right-hemisphere ventral model significantly predicts Language Story Median RT, while the left-hemisphere equivalent does not survive partial correlation.
...
## 5.6.1 Limitations
We found a series of limitations regarding this study, following the nature of RSFC, the atlas used, the tasks of the HCP and the underlying literature. 

### Indirect Nature of Resting-State fMRI
Analysing brain conenctivity patterns with RSFC can be compared to analyse the amount of students arriving at buildings on the campus by looking at the amount of paved grassways in the middle of all buildings. It is a weak, but certain signal. It does not reveal directionality, nor causality, nor does it reveal which frequency or what other aspects drove this actiivty (outside noises, thoughts, stress). But assessing fMRI data from 812 participants makes this a real statistical powerful tool for assessing FC within the brain. This study should be a first step towards examining whether this hypothesis should be further investigated by MEG or well-designed and well-fitting behavioural tasks. 

### Limitations of Predicitve Modelling

**Predictive Modelling for the auditory what-stream.** Since language activity is strong in Broca's areas which are neighbour regions of IFJa, this could also produce some miss-interpreteations where actually IFJa only correlates with Broca's areas. We cannot see directionality in this study and therefore this will need more assessment. 
Also since this is a language task with humans listenting to a language they know we can also predcit, that this will activtae Broca's areas. However, it would be a lot more interesting to see activity in PFC while listening to a foreign language where we do not understand a word. A study in which we have to pay attention to a single trained word (e.g. "molus") this could possibly drive IFJa higher than the Broca's areas and therefore further support our hypothesis. This would also help explaining the ability of finding the correct key words in a noisy train station environment. 

**Predictive Modelling for the auditory where-stream.** A spatial hearing task, in which participants need to identify different sounds or words in space would not only activate the auditory where-stream, but could also determine whether we find a complementary dorsal language-stream as proposed by Rolls. This could be opossibly be tested by the differenc between known words, words in a foreign language or simple noises. 
Due to the nature of in-scanner tasks, a spatial hearing tasks could prove difficult in execution due to the gigh noise level inside the scanner. 

### Glasser Atlas Granularity
The Glasser-atlas is a state-of-the-art parcellation of the human brain. Still it is not perfect. In the meantime newer parcellations have been coming up differentiating between regions, we did not in this study -→ (to be proven). Ambiguious areas like A4/A5 are streched over a long distance showing activity for both where- and what-streams. This could imply that there are different neural populations within A4 and A5 serving different purposes. Dividing these areas into anterior and posterior parts might lead to a clearer pattern of this and future studies. 

### ROI Exclusion
The more ROIs are used, the less computational expressive the model gets (why? → to be proven).
...
[[Gold & Shadlen (2003)]] demonstrate that FEF continuously accumulates visual evidence from lower-level regions such as MT and LIP, integrating directional signals into developing saccade commands. An analogous paradigm could be designed for the auditory domain, testing whether FEF similarly receives a continuous stream of spatial signals from auditory cortex during sound-localisation decisions. Additionally, a task incorporating motor responses to hand movements rather than eye movements could address whether a distinct auditory how-stream exists independently of the where-stream, or whether both converge on a unified sensorimotor pathway.
...
Frühholz, S., Gschwind, M., & Grandjean, D. (2015). Bilateral dorsal and ventral fibre pathways for the processing of affective prosody identified by probabilistic fibre tracking. *NeuroImage*, *109*, 27–34. https://doi.org/10.1016/j.neuroimage.2015.01.016
