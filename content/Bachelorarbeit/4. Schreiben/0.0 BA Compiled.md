...
Statistical significance of FC estimates was assessed using the paired Wilcoxon signed-rank test, an alternative to the Gaussian-based paired t-test. This test was chosen because FC values derived from Pearson correlations do not necessarily follow a Gaussian distribution across subjects ([[Soyuhos, O., & Baldauf, D. (2022)]]) For the single-seed analyses, a one-tailed test was applied, evaluating whether connectivity within the seed region exceeded the mean whole-brain connectivity. For contrast analyses, a two-tailed test was used to determine whether conenctivity was significantly higher for one seed than for the other. 
...
To control for the elevated risk of false positives arising from testing all 360 cortical parcels somultaneously, all p-values were corrected for multiple comparisons using the False Discovery Rate (FDR) procedure ([[Benjamini, Y., & Hochberg, Y. (1995)]]. In contrast to the more restricting Bonferroni correction, FDR correction maintains higher statistical power by controlling the expected propotion of false positives among all rejected null hypotheses, rather than probability of any single false positives. A significance threshold of q < 0.05 (FDR-corrected) was applied in all analyses. 
