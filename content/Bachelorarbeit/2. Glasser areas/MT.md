---
title: MT
created: 2025-11-22
tags:
  - function/motion
  - stream/where
  - stream/safe
  - function/visual
modified: "2026-01-14"
---
# MT

**Stream:** 
**Classical equivalent:** 
**Mapped to Glasser:** ???

---

## MNI Coordinates
- **Right:**  
- **Left:**  


## Anatomical location
- next to [[MST]]
- 

## Function(s)
MT neurons have large receptive fields, which might provide a basis for determining whether a number of points (parts of an object) are moving together. Its cells are responsive to the direction and speed of moving stimuli. (Snowden et al., 1992). Neurons are arranged in “direction hypercolumns” within MT cortex.

Arrangement of columns in MT might be similar to the ice cube model of V1.
![[receptive fields MT.png]]

### neurons firing
Always fires once when the direction changes. 
So you can detect the change. 
![[MT neuron firing.png]]


## Connectivity
- only part of the visual 

### PFC targets


##  Summary


## Middle Temporal Visual Area (MT)


## see also
[[Lateral Intraparietal Cortex (LIP)]]

Tags: #cognitivescience/neuroscience #science 
*Superlink:* [[050 🧠Neuroscience]] 

## Source
[[Action&Cognition Motor System]]


vielleicht etwas, das zeigt, dass MT auch nur durch auditory stimuli angeregt werden kann 
https://www.sciencedirect.com/science/article/pii/S0926641002000617

und hier, dass MT/MST nicht von auditory streams beeinflusst werden: 
https://www.biorxiv.org/content/10.1101/204529v3.abstract

mein chat dazu: 
https://www.perplexity.ai/search/summarize-the-current-webpage-fuN_XTUPRluG_3WTDW1pTw#2

Created: 08-06-24 19:04

